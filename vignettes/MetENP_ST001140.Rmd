---
title: "Case study 3: Study ST001140"
...
ST001140 is the study of changes in the Canine Plasma Lipidome

```{r}
metdata = getmwstudies('ST001140', 'data')
...
For ex: in this study, we want to compare TreatmentGroup experimental methods, so we will compare Prednisolone and Tetracosactide samples
...
In this example, we show significant metabolite filtered according to pvalue threshold of 0.05 and log2 fold change of 0.5 and we use the normalization method "50percent" and using all the four analysis types

```{r}
stats_metabolites = significant_met(metabolomics_data=refmet_class, met_col="metabolite_name",analysis_type=c("Phospholipids, Chol. esters and Diacylglycerols","Sphingolipids","Derivatized Spingosine-1-phosphates","Triacylglycerols"), metadata=metadata, factor1='Prednisolone', factor2='Tetracosactide', factor_col='TreatmentGroup',sample_col='local_sample_id', p_adjust='fdr',normalization="50percent")
...
### Map kegg id to the significant metabolites

This function maps metabolite to kegg ids
...
Here the subject species is Dog, and the KEGG species annotation for KEGG is 'cfa'

```{r}
met_path = met_pathways(df_metenrichment = metenrichment, 'cfa')
...
In this example, the background set N is the total number of metabolites in the study, hence kegg_comp_path = FALSE, if we wanted to compare against total number of kegg compunds that are associated to pathways, we would use kegg_comp_path=TRUE and it will use data structure ls_path (list of all the KEGG compounds associated to pathway regardless of its presence in this study)

```{r}
load('C:/Users/bioso/Documents/MetENP/data/ls_path.RData')
load('C:/Users/bioso/Documents/MetENP/data/korg.RData')
kegg_es = path_enrichmentscore(met_path,sig_metabolite_kegg_id=sig_metabolite_kegg_id,ls_path=ls_path,refmet_class=refmet_class,sps='cfa',padj='fdr', kegg_comp_path=FALSE)
...
```{r}
met_gene_info = enzyme_gene_info (metenrichment, "cfa","sub_class")
