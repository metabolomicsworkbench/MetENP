---
title: "Case study 2: Study id: ST001308"
...
ST001308 is the NMR metabolomics on Salmonella enterica

```{r}
metdata = getmwstudies('ST001308', 'data')
...
```{r}
metadata = getmwstudies('ST001308', 'factors')
...
For ex: in this study, we want to compare Genotype experimental methods, so we will compare wild type with knockout mutants samples
...
In this example, we show significant metabolite filtered according to pvalue threshold of 0.05 and log2 fold change of 0.5 and we use the normalization method "50percent"

```{r}
stats_metabolites = significant_met(metabolomics_data=refmet_class, met_col="metabolite_name",analysis_type="NMR data", metadata=metadata, factor1='wild-type', factor2='rid1-knockout', factor_col='Genotype',sample_col='local_sample_id', p_adjust='fdr',normalization="half_of_min")
...
```{r}
met_path = met_pathways(df_metenrichment = metenrichment, 'seo')
...
In this example, the background set N is the total number of KEGG metabolites associated to pathway. The reason is that the total number of metabolites is less than K for some pathways.

```{r}
load('C:/Users/bioso/Documents/MetENP/data/ls_path.RData')
kegg_es = path_enrichmentscore(met_path,sig_metabolite_kegg_id=sig_metabolite_kegg_id,ls_path=ls_path,refmet_class=refmet_class,sps='seo',padj='fdr', kegg_comp_path=TRUE)
...
```{r}
met_gene_info = enzyme_gene_info (metenrichment, "seo","sub_class")
